...
TS_SIM_ACCOUNT_ID = os.getenv(
    "TS_SIM_ACCOUNT_ID",
    ""
).strip()

TS_LIVE_ACCOUNT_ID = os.getenv(
    "TS_LIVE_ACCOUNT_ID",
    ""
).strip()
...
def live_strategy_mode_selected(
    strategy_name
):
...
def confirm_live_market_order(
    ...
    url = (
        f"{TS_LIVE_API_BASE_URL}"
        f"/orderexecution/orderconfirm"
    )

    order = build_live_market_order(
        action
    )
    ...
    try:
        ...
        return (
            ...
            {
                "error":
                    f"LIVE Confirm Order request failed: {exc}"
            }
        )
...
def submit_live_market_order(
    ...
    if strategy_name not in ALLOWED_STRATEGIES:
        return (
            ...
            {
                "error":
                    "Blocked: strategy is not authorized for LIVE."
            }
        )
    ...
    if LIVE_TRADING_ENABLED != "YES":
        return (
            ...
            {
                "error":
                    "Blocked: LIVE_TRADING_ENABLED is not YES."
            }
        )

    if not live_strategy_mode_selected(
        strategy_name
    ):
        return (
            False,
    ...
    if not live_order_capability_ready():
        return (
            ...
            {
                "error":
                    "Blocked: LIVE order capability is not ready."
            }
        )
    ...
    try:
        ...
        return (
            ...
            {
                "error":
                    f"LIVE order request failed: {exc}"
            }
        )
...
def live_account_test():
    ...
    try:
        ...
        return jsonify({
            ...
            "error":
                f"LIVE account request failed: {exc}"
        }), 502
...
def live_position_test():
    ...
    if not TS_LIVE_ACCOUNT_ID:
        return jsonify({
            ...
            "error":
                "TS_LIVE_ACCOUNT_ID is not configured in Render.",

            "live_order_submission_enabled":
                False
        }), 503

    ok, quantity, body = get_soxl_live_position(
        access_token
    )
    ...
    if not ok:
        return jsonify({
            ...
            "error":
                "LIVE SOXL position query failed.",
...
def live_confirm_buy_test():
    ...
    ok, body = confirm_live_market_order(
        access_token,
        "BUY"
    )
    ...
    if not ok:
        return jsonify({
            ...
            "error":
                "TradeStation Confirm Order failed.",
    ...
    if live_strategy_mode_selected(
        ...
        if LIVE_TRADING_ENABLED != "YES":
            return jsonify({
                ...
                "message":
                    "LIVE_TRADING_ENABLED is not YES; "
                    "no order sent."
            }), 200
        ...
        if not live_order_capability_ready():
            return jsonify({
                ...
                "error":
                    "LIVE order capability is not ready."
            }), 503
        ...
        with live_order_lock:
            ...
            if position_qty not in {
                ...
                return jsonify({
                    ...
                    "error":
                        "LIVE safety block: SOXL account position "
                        "must be 0, 1, or 2 shares.",
            ...
            if (
                ...
                return jsonify({
                    ...
                    "message":
                        "LIVE BUY blocked: combined SOXL "
                        "maximum of 2 shares already reached.",
